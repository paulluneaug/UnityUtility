...
- Moved the `StableEnum`'s Odin Drawer to [a separate package](https://github.com/paulluneaug/UnityUtility_OdinInspector)
- Re-introduced a specific namespace for the Attributes
...
- Removed namespaces except for UnityUtility and UnityUtility.Editor

## [2.0.0] - 2026-04-07

### Removed

- Removed the test scrpts and Unity project to keep only the package
...
- Easing functions
- `RandomGenerator` and `RandomUtils` classes to hold the random part of the `Hasher` and `HashUtils` classes

### Modified

- Moved the extensions methods in separate files, classes and namespace
- Updated the `.editorconfig` file and Sample

## [1.10.2] - 2024-12-10

### Added

- Added an accessor to the prefab of the `ComponentPool` only in Editor mode

### Modified

- Fixed NullRef that occured sometimes when using the `ComponentPool`
